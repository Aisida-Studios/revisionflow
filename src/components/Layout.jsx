...
/* profile.subjects entries store the subject name under .name, not
   ...
   them. Search across real data only: nav pages, the subjects the user
   actually takes (profile.subjects — already in memory, no fetch), and
   that subject's syllabus topics (data/topics.js's getAllTopicsFlat —
   static, synchronous, no Firestore read). Topic results link straight
   to /topics/:topicId using the same buildTopicId() scheme
   TopicDetail.jsx reads its Firestore doc by. */
...
/* Bell panel: a real, live-subscribed notification feed (empty until
   something calls createNotification — see utils/notificationFeed.js and
   the badge-award / streak-freeze hooks in utils/firestore.js — no
   invented history) plus the real Web Push settings that were here
   before. */
