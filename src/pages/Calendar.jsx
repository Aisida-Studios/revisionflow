...
export default function Calendar() {
  ...
  // "Scheduled hours this week" always reflects the real current week (the one containing
  // today), not whichever week/month is being browsed — a stable "how am I doing this
  // week" stat regardless of calendar navigation. Counts everything scheduled for the
  // week (not only completed sessions) — this is "what's planned", same sense as the rest
  // of the calendar; tasks are excluded since they're not timed study sessions.
  const thisWeekDateSet  = new Set(getWeekDays(new Date()).map(d => format(d, 'yyyy-MM-dd')))
  const thisWeekHours    = hoursBySubject(sessions.filter(s => !s.isTask && s.date && thisWeekDateSet.has(s.date)))
  const thisWeekTotalMin = thisWeekHours.reduce((sum, [, m]) => sum + m, 0)
    ...
    <div className="fade-in">
      ...
      <div className="rf-cal-layout">
        ...
        <div className="card rf-side-card">
          {thisWeekHours.length > 0 && (
            <div className="rf-week-hours">
              <h4 style={{marginBottom:8}}><Clock size={14} color="var(--accent)"/> Scheduled hours this week</h4>
              <div className="rf-week-hours-list">
                {thisWeekHours.map(([subject, mins]) => (
                  <div key={subject} className="rf-week-hours-row">
                    <span className="rf-week-hours-dot" style={{background:subjectColour(subject)}}/>
                    <span className="rf-week-hours-subject">{subject}</span>
                    <span className="rf-week-hours-value">{formatDuration(mins)}</span>
                  </div>
                ))}
                <div className="rf-week-hours-row rf-week-hours-total">
                  <span className="rf-week-hours-subject">Total</span>
                  <span className="rf-week-hours-value">{formatDuration(thisWeekTotalMin)}</span>
                </div>
              </div>
            </div>
          )}
